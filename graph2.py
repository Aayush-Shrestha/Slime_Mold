...
class SlimeAgent:
    def __init__(self, start_node, graph_positions):
...
class SlimeMoldMST:
    def __init__(self, num_nodes=26, num_sources=None):
    ...
    def initialize_agents(self):
        """Initialize slime agents at random source nodes"""
        source_nodes = random.sample(range(self.num_nodes), self.num_sources)
        self.agents = []
        
        for source in source_nodes:
            agent = SlimeAgent(source, self.positions)
            self.agents.append(agent)
            
        print(f"Initialized {len(self.agents)} agents at nodes: {source_nodes}")
        return source_nodes
